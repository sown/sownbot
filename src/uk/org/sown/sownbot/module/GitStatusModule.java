...
public class GitStatusModule extends InteractiveSownBotModule {
	enum GitCommand { list, ack, blame };
	
	private static final String keyw_git = "git";
	private static final String cmd_list = "list";
	private static final String cmd_ack = "ack";
	private static final String cmd_blame = "blame";
	private static final String[] usage_keyw_git  = new String[] {"Usage: !git (list <args> | ack <args> | blame <args)"};
	private static final String[] usage_cmd_list  = new String[] {"Usage: !git list [host] [repo path]"};
	private static final String[] usage_cmd_ack   = new String[] {"Usage: !git ack <file path> in <repo path> on <host>"};
	private static final String[] usage_cmd_blame = new String[] {"Usage: !git blame <user> <file path> in <repo path> on <host>"};
	
	
	private static final String moduleName = "GitStatus";
	private static final String[] keywords = new String[] {
		keyw_git
	};
	
	private static final String[][][] keywordUsage = new String[][][] {
		{ usage_keyw_git, usage_keyw_git }
	};
	...
	public GitStatusModule(SownBot bot, String host, String database, String username, String password) {
			...
			throws SownBotModuleException
	{
		...
		try {
			gitCommand = GitCommand.valueOf(command);
	...
	private Collection<? extends SownBotAction> processListRequest(SownBot sownBot, Message msg, String args) {
		...
		String queryString = 
			"SELECT host_name, repo_path, file_path, blamed_user FROM git_status";
			...
			throws SownBotModuleException
	{
		...
		if (parts.length == 0)
			return sendUsage(msg, keyw_git, cmd_ack);
		
		if (parts.length == 3) {
			queryString = "UPDATE git_status SET last_acknowledge_time = TIMESTAMPADD(DAY, 1, CURRENT_TIMESTAMP) WHERE file_path LIKE ? AND repo_path LIKE ? AND host_name LIKE ?;";
			queryArgs = parts;
	...
	private Collection<? extends SownBotAction> processBlameRequest(SownBot sownBot, Message msg, String args) {
		...
		if (parts.size() == 4) {
			queryString = "UPDATE git_status SET blamed_user = ? WHERE file_path LIKE ? AND repo_path LIKE ? AND host_name LIKE ?;";
